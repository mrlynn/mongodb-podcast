...
const Meta = ({ show, staticPage }) => (
  <Head>
    {show ? (
      <>
        <title>
          {show.title} — MongoDB Podcast {show.displayNumber}
        </title>
        ...
        <meta
          property="og:title"
          content={`${show.title} — MongoDB Podcast ${show.displayNumber}`}
        />
        <meta
          property="og:url"
          content={`https://mongodbpodcast.com/show/${show.displayNumber}/${slug(show.title)}`}
        />
      </>
    ) : (
      <title>
        {staticPage && staticPage.title && `${staticPage.title} – `}MongoDB
        Podcast
      </title>
    )}
  </Head>
